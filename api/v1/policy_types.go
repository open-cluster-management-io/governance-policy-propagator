...
type PolicyTemplate struct {
	...
	// IgnorePending is a boolean specifying whether to ignore this template's Pending status when
	// calculating the overall policy status. The default is false to not ignore a pending status.
...
type PolicyDependency struct {
	...
	// Name is the name of the dependent object to be checked
	Name string `json:"name"`

	// Namespace is the namespace of the dependent object to be checked (optional)
	Namespace string `json:"namespace,omitempty"`

	// Compliance is the ComplianceState of the dependent object (at path .status.compliant) to be
	// checked.
...
type PolicySpec struct {
	// Disabled is a boolean providing the ability to enable and disable the policy. When disabled,
	...
	// This setting is useful if there is tracking metadata that should only exist on the root policy.
	// Defaults to `true`.
	//
...
type Placement struct {
	...
	// apps.open-cluster-management.io API group) that has been bound to the policy.
	...
	// API group) that has been bound to the policy.
...
type ComplianceHistory struct {
	// LastTimestamp is the timestamp of the event that reported this message.
	LastTimestamp metav1.Time `json:"lastTimestamp,omitempty" protobuf:"bytes,7,opt,name=lastTimestamp"`

	// Message is the compliance message resulting from evaluating this policy resource.
	Message string `json:"message,omitempty" protobuf:"bytes,4,opt,name=message"`

	// EventName is the name of the event attached to this message.
...
type PolicyStatus struct {
	...
	// field is only used in the root policy on the hub.)
	Placement []*Placement `json:"placement,omitempty"`

	// Status is a list of managed clusters and the current compliance state of each one.  (This
	// status field is only used in the root policy on the hub.)
	Status []*CompliancePerClusterStatus `json:"status,omitempty"`

	// REPLICATED POLICY STATUS FIELDS

	// ComplianceState reports the observed status resulting from the policy's definition(s). (This
